...
class Senko:
    ...
    def _check_hash(self, latest, local):
        print("Latest:")
        print(latest)
        print("==============")
        print("Local:")
        ...
        latest_hash = uhashlib.sha1(latest.encode())
        local_hash = uhashlib.sha1(local.encode())

        latest = latest_hash.digest()
        local = local_hash.digest()

        if str(latest) == str(local):
            return True
        else:
            return False
